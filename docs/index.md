...
::: tips
**不是必须做的，但会优化你的编码体验的事**
实际上，记事本虽然可以用来编写代码，但用起来可能并不是那么方便。如果你不怕一点点麻烦，可以进入 [http://www.sublimetext.com/3](http://www.sublimetext.com/3) 下载 sublime text，用它来代替记事本。
:::
...
这一行不是代码，它只是一个小提醒，无论它在或不在，都不会影响代码的运行。你可以把它删掉，也可以将它保留，甚至可以多写点什么，只要不要忘记：假如你要换行写，请以 // 开头。
...
OK，那我们快来试一下吧！在 ==// 在这里写你的代码== 这一行的下面一行输入：
...
以此来将人的默认的样子（一个绿色方块）改变为一个嚣张奔跑的人的样子。

现在这个小人儿可以自由活动啦！你可以使用键盘上的 a、d、w 来控制小人行走的方向和跳跃。

::: tips
**小提示**
你可以打开[图案图鉴](https://www.notion.so/f99b9ae61de94e1f8d703e9e8266a0c2)来浏览和挑选目前可用的图案。
:::
...
::: tips
**小提示**
打开[详细说明书](/?page=specification)后，你可以按 ctrl f 来搜索 automove.left 所在的位置并直接到达那里。
:::
...
留一个小练习：请你通过查阅附录的[图案图鉴](https://www.notion.so/f99b9ae61de94e1f8d703e9e8266a0c2)和[详细说明书](/?page=specification)，来实现那个飞来飞去的绿色龟壳。如果遇到困难，可以用记事本打开 ==example.html== 查看提示。
...
在 if 后的括号内写下需要进行判断的代码，如果这条代码的执行结果是 ==true== ，即“没错，就是这样”，那么第一个花括号中的代码会被执行；如果这条代码的执行结果是 ==false== ，即“不是这样”，那么就会进入“否则”，即第二个花括号中的代码会被执行。

再来看 if 的括号中的代码：

```jsx
human.istouched(fire1)
```

我们的积木提供一个名为 ==istouched== 的动作，它可以用于判断这个积木和另一个积木是否产生 ==接触== ，如果产生接触，那么这个动作的执行结果是 true，反之为 false。所以上述代码的含义是：判断 human 和 fire1 是否产生接触。同时，积木还提供一个名为 ==isoverlapping== 的动作，它是用于判断这个积木和另一个积木是否产生 ==重叠== 。关于这两个动作的具体区别，你可以通过查阅[详细说明书](/?page=specification)来了解。
...
我们将上述流程图实现为伪代码（伪代码是并非可以真正运行的代码，但它能够很容易被改为可以真正运行的代码）：
...
当小人儿的状态为正在与石头接触时，我们让 state 等于 true，反之，让它等于 false；当 stone 需要知道小人儿的状态时，就访问 state 看它是否等于 true，如果是，那么就同步改变小人儿的位置。完整的代码如下：
